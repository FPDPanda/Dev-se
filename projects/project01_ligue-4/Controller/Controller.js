class Controller {
  ...
  checksWinner() {
    ...
    if (!data[3]) {
      for (let l = 0; l < this.matrix.length; l++) {
        for (let c = 0; c < this.matrix.length; c++) {
          if (this.matrix[l][c] == this.nameBall(this.ball.toggleColor[0])) {
            data[0] += 1;
            data[2] = this.ball.toggleColor[0];
          } else data[0] = 0;
          if (this.matrix[l][c] == this.nameBall(this.ball.toggleColor[1])) {
            data[1] += 1;
            data[2] = this.ball.toggleColor[1];
          } else data[1] = 0;
    ...
    if (!data[3]) {
      ...
      for (let l = 0; l < this.matrix.length; l++) {
        for (let c = 0; c < this.matrix.length; c++) {
          if (this.matrix[c][l] == this.nameBall(this.ball.toggleColor[0])) {
            data[0] += 1;
            data[2] = this.ball.toggleColor[0];
          } else data[0] = 0;
          if (this.matrix[c][l] == this.nameBall(this.ball.toggleColor[1])) {
            data[1] += 1;
            data[2] = this.ball.toggleColor[1];
          } else data[1] = 0;
          if (data[0] > 3 || data[1] > 3) {
        ...
        for (let c = 0; c < this.matrix.length; c++) {
          ...
          if (l < 4 && c < 4) {
            for (let i = 0; i < 4; i++) {
              if (
                this.matrix[l + i][c + i] ==
                this.nameBall(this.ball.toggleColor[0])
              ) {
                data[0] += 1;
                data[2] = this.ball.toggleColor[0];
              } else data[0] = 0;
              if (
                this.matrix[l + i][c + i] ==
                this.nameBall(this.ball.toggleColor[1])
              ) {
                data[1] += 1;
                data[2] = this.ball.toggleColor[1];
              } else data[1] = 0;
    ...
    if (tag) {
      tag.forEach((el) => {
        el.addEventListener("click", (e) => {
          ...
          if (this.runningGame && this.checksWinner()[3]) {
            setTimeout(() => {
              this.runningGame = false;
              this.gameFinished(this.checksWinner()[2]);
            }, 150);
          }
        });
      });
  ...
  mainLoop() {
    ...
    loop = setInterval(() => {
      if (this.runningGame) {
        ...
        ball &&
        ball[ball.length - 1].offsetTop <
          this.pipe.offsetTop + this.pipe.offsetHeight
